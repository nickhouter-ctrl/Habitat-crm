...
const account={id:"existing",contactId,email:"customer@example.com",status:"active",websiteAccess:true,businessName:"Dealer BV",vatNumber:"NL123456789B01"};
const form=()=>{const f=new FormData();f.set("system","windows");return f;};
beforeEach(()=>{vi.clearAllMocks();m.guard.mockResolvedValue({id:"admin"});m.contact.mockResolvedValue({id:contactId,email:account.email,name:"Customer"});m.matches.mockResolvedValue([account]);m.update.mockReturnValue({set:()=>({where:vi.fn().mockResolvedValue([])})});});
