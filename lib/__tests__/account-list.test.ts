...
const row=(id:string,extra:Partial<AccountListRow>={}):AccountListRow=>({id,email:id+'@example.com',businessName:id,contactName:null,status:'active',windowsApproved:false,windowsAccess:false,createdAt:new Date('2026-01-01'),lastLoginAt:null,...extra});
...
describe('account page scope and filters',()=>{
 ...
 it('shows all shared website identities on the website page',()=>{expect(accountList(rows,false,{}).scoped).toHaveLength(3);});
