...
export async function ContactOnlineAccess({contactId,email}:{contactId:string;email:string|null}) {
 ...
 const invitation=(system:"website"|"windows")=><form action={createAccountForContact.bind(null,contactId)} className="mt-3 flex flex-wrap items-center gap-2"><input type="hidden" name="system" value={system}/>{system==="windows"&&<><Input name="businessName" placeholder="Bedrijfsnaam" aria-label="Bedrijfsnaam" defaultValue={account?.businessName||""} required/><Input name="vatNumber" placeholder="BTW-/IVA-nummer" aria-label="BTW-/IVA-nummer" defaultValue={account?.vatNumber||""} required/></>}{!account&&system==="website"&&<Select name="tier" aria-label="Prijsniveau website" defaultValue="particulier"><option value="particulier">Particulier</option><option value="aannemer">Zakelijk</option></Select>}<SubmitButton size="sm" variant="secondary">{account ? system==="windows" ? "Gegevens opslaan en Windows toestaan" : "Bestaand account koppelen" : system==="windows" ? "Windows-account uitnodigen" : "Websiteaccount uitnodigen"}</SubmitButton></form>;
 return <Card id="online-toegang" className="mb-5"><details><summary className="cursor-pointer px-5 py-4"><span className="mr-4 font-semibold">Online toegang</span><span className="inline-flex flex-wrap gap-2 align-middle"><Badge tone={account?.websiteAccess&&account?.status==="active"?"success":"neutral"}>Website: {!account?.websiteAccess?"geen toegang":account?.status==="active"?"actief":account?.status==="pending"?"activatie nodig":account?.status==="suspended"?"geblokkeerd":"geen account"}</Badge><Badge tone={allowed?"success":"neutral"}>Windows: {allowed?"toegestaan":"geen toegang"}</Badge></span><span className="ml-3 text-xs text-muted">Beheren</span></summary>
 ...
 <section className="rounded-lg border border-border p-4"><h3 className="mb-2 font-semibold">Habitat One-website</h3><Badge tone={!account?.websiteAccess ? "neutral" : account.status==="active" ? "success" : account.status==="pending" ? "warning" : "danger"}>{!account?.websiteAccess ? "Geen website-toegang" : account.status==="active" ? "Actief" : account.status==="pending" ? "Wacht op activatie" : "Account geblokkeerd"}</Badge>
 <p className="mt-2 text-sm text-muted">{account?.email || email || "Voeg eerst een e-mailadres toe aan dit contact."}</p>
 {linked&&account&&<form action={setWebsiteAccess.bind(null,account.id,!account.websiteAccess)} className="mt-3"><SubmitButton size="sm" variant={account.websiteAccess?"ghost":"secondary"}>{account.websiteAccess?"Website-toegang intrekken":"Website-toegang toestaan"}</SubmitButton></form>}
 ...
 {account&&linked&&(!allowed&&(!account.businessName||!account.vatNumber)) ? invitation("windows") : account&&linked ? <form className="mt-3" action={setWindowsAccess.bind(null,account.id,!allowed)}><SubmitButton size="sm" variant={allowed?"ghost":"secondary"}>{allowed?"Windows-toegang intrekken":"Windows-toegang toestaan"}</SubmitButton></form> : email ? invitation("windows") : null}
