...
import { activities, companies, contacts, deals, deliveries, documents, holdedSyncMap, products, projectBudgetLines, projectPayments, projects, type DocumentLineItem, type DocumentPhase } from "@/lib/db/schema";
 ...
 */
async function richtProjectInNaAkkoord(estimateId: string, userId: string | null) {
...
export async function setDocumentStatus(id: string, formData: FormData) {
  ...
  // Akkoord op een gecalculeerde verbouwings-offerte → project inrichten
  // (aanneemsom, contractdatum, budgetregels). Best-effort: een fout hier mag
  // de statuswijziging zelf niet blokkeren.
  if (status === "accepted" && doc.kind === "estimate") {
    try {
      await richtProjectInNaAkkoord(id, user?.id ?? null);
    } catch (e) {
      console.error("[setDocumentStatus] project inrichten na akkoord mislukt:", e);
    }
  }
