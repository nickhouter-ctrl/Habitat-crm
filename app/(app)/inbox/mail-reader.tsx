...
import { emailInbox, inboxSuggestions, mailAttachments } from "@/lib/db/schema";
...
export async function MailReader({ id, reply, replyResult, backHref, mailHref }: { id: string; reply: boolean; replyResult: string; backHref: string; mailHref: string }) {
  const [mail, suggestion, attachments, session] = await Promise.all([
    ...
    db.select({ id: mailAttachments.id, filename: mailAttachments.filename, sizeBytes: mailAttachments.sizeBytes })
      .from(mailAttachments).where(eq(mailAttachments.emailId, id)).orderBy(asc(mailAttachments.filename)),
    auth(),
  ]);
